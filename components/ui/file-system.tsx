...
export type FileSystemProps = {
  ...
  /**
   * Show file extensions in displayed names. When `false`, a regular file's
   * trailing extension is hidden in the Icons, Columns, and Gallery views (the
   * List view always shows full names). Defaults to `true`.
   */
  showFileExtensions?: boolean
  /** Folder prefix to open initially, e.g. `"invoices/"`. */
  defaultPath?: string
  /** Called with the currently visible folder prefix (and whenever it changes). */
...
const ShowFileExtensionsContext = React.createContext(true)

function formatEntryName(entry: FileSystemEntry, showFileExtensions: boolean) {
  if (showFileExtensions || entry.kind === "folder") return entry.name
  const dotIndex = entry.name.lastIndexOf(".")
  return dotIndex <= 0 ? entry.name : entry.name.slice(0, dotIndex)
}

function useFormatEntryName() {
...
function createFileSystemFilterId() {
